...
void Stitch(Frame & frame)
{
	...
	vector<Mat> images;
	Frame framestitch;
	openImage(framestitch);
	framestitch.getImage(imgstich);
	imgstitchSent = imgstich.clone();
	frame.updateImage();
	frame.getImage(img);
	imgCopy = img.clone();
	imgSent = imgCopy.clone();
	images.push_back(imgSent);
	images.push_back(imgstitchSent);
	Section* rightColumn0 = new Section(cv::Mat(920, 250, CV_8UC3, Scalar(240, 240, 240)), 3);
